...
import torch
from helper import capitalization_analysis
import sys
...
class NN(nn.Module):
    ...
    def get_train_tensors(self, words, tags=[],  word2id=None, tag2id=None, suffix2id=None):
        ...
        word_tensor = dtype(len(words), self.batch_size, 1+self.window_size*2)
        tag_tensor = dtype(self.batch_size, len(words))
        cap_tensor = dtype(len(words), self.batch_size, 1+self.window_size*2)
        suf_tensor = dtype(len(words), self.batch_size, 1+self.window_size*2)
