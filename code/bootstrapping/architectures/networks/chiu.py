...
class Network(nn.Module):
    ...
    def __init__(self, vocab_size, number_tags, hidden_size=275,
                 batch_size=1, embedding_dim=50, retrain_embeddings=True,
                 gpu=False, include_cap=False, dropout=0.68):
        ...
        self.lstm_forward = nn.LSTM(input_size=embedding_dim,
                                    hidden_size=hidden_size, num_layers=1, batch_first=True,
                                    dropout=dropout)
        self.lstm_backward = nn.LSTM(input_size=embedding_dim,
                                     hidden_size=hidden_size, num_layers=1, batch_first=True,
                                     dropout=dropout)
    ...
    """
    def forward(self, input, cap_tensor):
        input_list = input.data.tolist()
        input_list = [l[::-1] for l in input_list]
        input_backward = Variable(self.long_type(input_list))

        cap_list = cap_tensor.data.tolist()
        cap_list = [l[::-1] for l in cap_list]
        cap_backward = Variable(self.long_type(cap_list))
        ...
        if self.include_cap:  # concatenate capitalization/suffix info
            f_cap_emb = self.cap_embeddings(cap_tensor)
            b_cap_emb = self.cap_embeddings(cap_backward)
            f_emb = torch.cat((f_emb, f_cap_emb), 2)
            b_emb = torch.cat((b_emb, b_cap_emb), 2)
        ...
        output_tensor = Variable(self.float_type(input.shape[0], input.shape[1], self.number_tags))
    ...
    def get_train_tensors(self, words, tags=[], word2id=None, tag2id=None):
        ...
        word_tensor = torch.LongTensor(self.batch_size, len(words))
        tag_tensor = torch.LongTensor(self.batch_size, len(words))
        cap_tensor = torch.LongTensor(self.batch_size, len(words)).fill_(0)
        ...
        for pos, (word_list, tag_list) in enumerate(zip(words, tags)):
            for batch, (word, tag) in enumerate(zip(word_list, tag_list)):
                ...
                if self.include_cap:
                    cap_tensor[batch][pos] = capitalization_analysis(word)
        if self.gpu and torch.cuda.is_available():
            return word_tensor.cuda(), tag_tensor.cuda(), cap_tensor.cuda()
        else:
